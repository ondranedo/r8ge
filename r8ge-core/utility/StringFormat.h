    ...
    namespace utility {
        ...
        public:
            ...
            using ValidType = std::variant<
                    int, float, std::string_view, char, unsigned long, std::vector<std::string>
                    >;
            using ValidList = std::initializer_list<ValidType>;
            StringFormat(std::string_view form, const ValidList& list);
            StringFormat(std::string_view str);
            StringFormat();
